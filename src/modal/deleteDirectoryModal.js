...
const DeleteFolderModal = ({ visible, onClose, folder }) => {
    ...
    const handleDeleteFolder = async () => {
        try {
            const folderExists = await FileSystem.getInfoAsync(FOLDERS_DIRECTORY_PATH + '/' + folder);
            if (folderExists.exists) {
                await FileSystem.deleteAsync(FOLDERS_DIRECTORY_PATH + '/' + folder, { idempotent: true });
                onClose();
            } else {
        ...
        >
            <View style={styles.modalContainer}>
                <View style={styles.modalContent}>
                    <Text>Are you sure delete dirictory?</Text>
